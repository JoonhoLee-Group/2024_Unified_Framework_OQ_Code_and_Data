...
rhor_noisy = np.zeros((4, 4, len(tarr)), dtype="complex")
U = numpy.zeros((4, 4, N), dtype="complex")  # propagator
U_noisy = numpy.zeros((4, 4, N), dtype="complex")  # propagator
for i in range(0, N):  # this fits the propagators from time series data
    rhor[:, 0, i] = A1[:, i]
    rhor[:, 1, i] = A2[:, i]
    rhor[:, 2, i] = A3[:, i]
    rhor[:, 3, i] = A4[:, i]
    # rho = A4[:, i].reshape(2, 2)
    # e, v = numpy.linalg.eig(rho)
    # print(i, e)
    rhor_noisy[:, 0, i] = A1[:, i] + numpy.random.normal(scale=sigma)
    rhor_noisy[:, 1, i] = A2[:, i] + numpy.random.normal(scale=sigma)
    rhor_noisy[:, 2, i] = A3[:, i] + numpy.random.normal(scale=sigma)
    rhor_noisy[:, 3, i] = A4[:, i] + numpy.random.normal(scale=sigma)
    U[:, :, i] = rhor[:, :, i] @ U0inv
    U_noisy[:, :, i] = rhor_noisy[:, :, i] @ U0inv
...
rmax = 4
Ms = generate_Mnew(U_noisy, rmax)
Uapprox = numpy.zeros_like(U_noisy)
Uapprox[:, :, : rmax + 1] = U_noisy[:, :, : rmax + 1]
for it in range(rmax + 1, N):
    Uapprox[:, :, it] = comptue_Uk0(Uapprox, Ms, it, rmax)
A1t_noisy = np.einsum("ijt,j->it", Uapprox, U0[:, 0])

import matplotlib.pyplot as plt

plt.plot(A1t[0, :-1], ls="--", label=f"rmax={rmax} (noise-free)")
plt.plot(A1t_noisy[0, :-1], ls="--", label=f"rmax={rmax}, sigma={sigma} (noisy)")
