...
#ax.plot(np.arange(2,kmax+1)*wantdelt,MK[1,2,2:],color="k", ls="none",marker='.',label=r'$\|\mathcal{K}_{N \Delta t}\|$')
ax.semilogy(np.arange(2,kmax+1)*wantdelt,Mnorm[2:],color="k", ls="none",marker='.',label=r'$\|\mathcal{K}_{N}\|$')
...
import matplotlib.pyplot as plt
from pyeom.inverse.inverse import *
filename = f'heomforK0.145.00.5/tmax45.0beta5alpha0.5s0.5initial1.h5'

h5 = h5py.File(filename,'r')
#kmax=14
A1 = np.transpose(h5["rho"])

filename = f'heomforK0.145.00.5/tmax45.0beta5alpha0.5s0.5initial2.h5'

h5 = h5py.File(filename,'r')
#kmax=14
A2 = np.transpose(h5["rho"])

filename = f'heomforK0.145.00.5/tmax45.0beta5alpha0.5s0.5initial3.h5'

h5 = h5py.File(filename,'r')
#kmax=14
A3 = np.transpose(h5["rho"])

filename = 'heomforK0.145.00.5/tmax45.0beta5alpha0.5s0.5initial4.h5'
...
ax.semilogy(range(2,14+1)*delt,x[1:14], color='#5E091A', ls="none",marker='x')
...
plt.xticks(fontsize=15)
plt.yticks(fontsize=15)
plt.savefig(f"KINorm0505lots",dpi=1000,bbox_inches="tight")
plt.show()