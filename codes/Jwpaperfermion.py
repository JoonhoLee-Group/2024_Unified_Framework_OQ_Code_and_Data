...
def J(xs):  # Spectral density (Ohmic bath with exponential cutoff)
    ...
    #output = ga / ((xs-w0)**2 + ga**2)+ga / ((xs-3*w0)**2 + ga**2)
    #output[xs <= 0] = ga / ((xs-w0)**2 + ga**2)
    output = ga / ((1+(np.exp(10/ga*(xs-wcut))))*(1+(np.exp(-10/ga*(xs+wcut)))))
    #output = ga * xs * w0 ** 2 / ((xs**2-w0**2)**2+ga**2*xs**2)+ga / ((xs-w0)**2 + ga**2)+ga / ((xs-3*w0)**2+ ga**2)
    return output
for kmax in [8,12,16]:
    ...
    for iii in enumerate(om):
        ...
        yy[iii[0]]=1/(2*np.pi)*J(om[iii[0]])/(om[iii[0]]**2)*(1+tanh(beta*hbar*om[iii[0]]/2))*(1-np.exp(-1j*om[iii[0]]*delt))
    etanul=trapz(yy,om)
    for difk in np.linspace(1,kmax,kmax,dtype='int'):

        for i in enumerate(om):
            #np.exp(beta*hbar*om[i[0]]/2)/(math.sinh(beta*hbar*om[i[0]]/2))
            yy[i[0]]=2/(1*np.pi)*J(om[i[0]])/(om[i[0]]**2)*(1+tanh(beta*hbar*om[i[0]]/2))*((math.sin(om[i[0]]*delt/2))**2)*np.exp(-1j*om[i[0]]*delt*(difk))
    ...
    for i in enumerate(om):
            #np.exp(beta*hbar*om[i[0]]/2)/(math.sinh(beta*hbar*om[i[0]]/2))
        yy[i[0]]=2/(1*np.pi)*J(om[i[0]])/(om[i[0]]**2)*(1+tanh(beta*hbar*om[i[0]]/2))*((math.sin(om[i[0]]*delt/2))**2)*np.exp(- 1j*om[i[0]]*delt*(0))
    ...
    for w in  enumerate(ww):
        for iii in enumerate(new_x):
            ...
            #yyy[iii[0]]= (interpolated_y[iii[0]])*np.cos(delt*iii[1]*w[1])*(np.pi/2)
        F[w[0]]=trapz(yyy,new_x)/(2*np.pi)
        JJ[w[0]]=F[w[0]]*np.pi*w[1]**2 *np.sinh(beta*w[1]/2)/(2*np.sin(w[1]*delt/2)**2*np.exp(beta*w[1]/2))*(delt)
    i=kmax
    if kmax==8:
        plt.plot(ww,np.real(JJ),label='extracted (order 8)',color=(1-i/40,1-i/40,1-i/40))#
    elif kmax==16:
        plt.plot(ww,np.real(JJ),label='extracted (order 16)',color=(1-i/40,1-i/40,1-i/40))#
    #elif kmax>20:
        #plt.plot(ww,np.real(JJ),label=f'$\eta$ inverse Fourier (order {kmax})',alpha=0.7,color='blue')#
    else:
        plt.plot(ww,np.real(JJ),color=(1-i/40,1-i/40,1-i/40))#
    #print(beta)
    #print(delt)
for kmax in [20,30,40]:
    etakkdp=np.zeros(kmax+1,dtype='complex')
    etakkdm=np.zeros(kmax+1,dtype='complex')
    for iii in enumerate(om):
        #np.exp(beta*hbar*om[iii[0]]/2)/(math.sinh(beta*hbar*om[iii[0]]/2))
    ...
    for w in  enumerate(ww):
        for iii in enumerate(new_x):
            ...
            #yyy[iii[0]]= (interpolated_y[iii[0]])*np.cos(delt*iii[1]*w[1])*(np.pi/2)
        F[w[0]]=trapz(yyy,new_x)/(2*np.pi)
        JJ[w[0]]=F[w[0]]*np.pi*w[1]**2 *np.cosh(beta*w[1]/2)/(2*np.sin(w[1]*delt/2)**2*np.exp(beta*w[1]/2))*(delt)
    i=kmax
    if kmax==2:
        ...
        plt.plot(ww,np.real(JJ),color=(1-i/40,1-i/40,1-i/40),ls='dotted')#
    #print(beta)
    #print(delt)
...
plt.savefig(f"plots/Jw{kondo}{ohmicity}c.png",dpi=1000)
plt.show()
