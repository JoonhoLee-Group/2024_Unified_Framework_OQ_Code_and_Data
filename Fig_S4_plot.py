...
import h5py
import numpy as np
file_path = 'Data\Fig S4 Data\Driven_HEOM_results\driven_heom_d_eps_1.out'
...
plt.plot(time,y,color='red',ls='dashed',marker='o',markevery=15)
plt.plot(time,y1,color='red',ls='dashed',marker='x',markevery=15)
plt.plot(time,y2,color='red',ls='dashed',marker='+',markevery=15)
filename='Data\Fig S4 Data\drivendata/d.h5'
...
plt.plot(tarr,M[0,:],color='#808080',marker='o',ls='dotted',markevery=20,label='Dyck, order 4')
...
filename='Data\Fig S4 Data\drivendata/td25.h5'
h5 = h5py.File(filename,'r')
#kmax=14
kmax=4
N=200
...
plt.plot(tarr,MM[0,:],color='#808080',marker='o',markevery=20,label='Dyck, order 4 + crest term upto 5')
...
filename='Data\Fig S4 Data\drivendata/td2.h5'
h5 = h5py.File(filename,'r')
#kmax=14
kmax=4
N=200
...
for i in np.arange(kmax+1,N):
    MM[:,i]=np.matmul(UG[:,:,i],np.transpose([1,0,0,0]))
plt.plot(tarr,MM[0,:],color='black',marker='o',markevery=20,label='Dyck, order 4 + crest term upto 6')
...
plt.xlim([0,20])
plt.legend()
#plt.savefig("driven5.png",dpi=1000)
plt.show()