...
filename = f'Data/Fig 2 Data/jobgqme0.51.0/dyn0.5beta5delt0.1ohmicity1.0.h5'
...
MMM=np.zeros((4,4,kmax+1),dtype = 'complex')
...
ax.semilogy(np.arange(2,kmax+1)*wantdelt,Mnorm[2:],color="k", ls="none",marker='.')
...
filename = f'Data/Fig 2 Data/heompaper10.115.0/heompaper10.115.0/tmax15.0beta5alpha0.5s1initial2.h5'

h5 = h5py.File(filename,'r')
#kmax=14
A2 = np.transpose(h5["rho"])

filename = f'Data/Fig 2 Data/heompaper10.115.0/heompaper10.115.0/tmax15.0beta5alpha0.5s1initial3.h5'

h5 = h5py.File(filename,'r')
#kmax=14
A3 = np.transpose(h5["rho"])

filename = 'Data/Fig 2 Data/heompaper10.115.0/heompaper10.115.0/tmax15.0beta5alpha0.5s1initial4.h5'
...
ax.plot(t[2:],Mnormex[2:],color=(0,0,0),alpha=1, ls="solid",label="HEOM $\mathcal{K}_{12}^N$",markersize=7)
...
filename="Data/Fig 2 Data/crestcrest6/dyn0.7853981633974483ohmicity1.0.h5"

h5 = h5py.File(filename,'r')
#kmax=14
sett = h5["sett"]  # VSTOXX futures data
nnn = h5["norm"]
nnnn=np.zeros(17,dtype='complex')
aaa=np.zeros(17,dtype='complex')
bbb=np.zeros((4,4,18),dtype='complex')
#plt.plot(range(2,18)*delt,list(sett[1:]),alpha=0.9,color='black', ls="dashed",label=r'$\|\mathcal{K}_{N,crest}\|$')
for i in range(17):
    bbb[:,:,i]=sett[:,:,i]
    nnnn[i]=nnn[i]
h5.close()
...
plt.savefig(f"Fig1b.eps",format='eps',bbox_inches="tight")
